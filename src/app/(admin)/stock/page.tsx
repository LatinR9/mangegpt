...
export default function StockPage() {
  ...
  return (
    <div>
      ...
      <section className="mb-6 grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-8">
        {[
          ...
          <Card>
            ...
            <CardContent>
              {filtered.length === 0 ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{t("empty")}</div> : (
                ...
                <div className="grid gap-3 lg:hidden">{filtered.map((account) => { const app = apps.find((item) => item.id === account.app_id); return <div key={account.id} className="min-w-0 rounded-xl border border-slate-800 bg-slate-950/70 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-base font-semibold">{account.label}</p><p className="mt-1 truncate text-sm text-muted-foreground">{app?.name ?? "-"}</p></div><StatusBadge status={t(account.status)} /></div><div className="mt-4 space-y-3 text-sm"><div><p className="text-muted-foreground">Email</p><div className="mt-1 grid grid-cols-[minmax(0,1fr)_auto] gap-2"><span className="min-w-0 truncate text-slate-200">{account.login_email}</span><Button variant="outline" onClick={() => copyText(`${account.id}-email`, account.login_email)}>{copied === `${account.id}-email` ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</Button></div></div><div><p className="text-muted-foreground">Password</p><div className="mt-1 grid grid-cols-[minmax(0,1fr)_auto] gap-2"><code className="min-w-0 truncate rounded-md border bg-slate-950 px-2 py-2 text-xs text-blue-100">{maskedPassword}</code><Button variant="outline" onClick={() => copyText(`${account.id}-password`, account.password ?? "")}>{copied === `${account.id}-password` ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</Button></div></div><div className="grid grid-cols-2 gap-3"><div><p className="text-muted-foreground">Expiry</p><p>{account.expiry_date ? formatDate(account.expiry_date) : "-"}</p></div><div><p className="text-muted-foreground">Price</p><p>{formatCurrency(account.selling_price)}</p></div></div></div><div className="mt-4 grid gap-2 min-[390px]:grid-cols-3"><Button variant="outline" onClick={() => updateStatus(account.id, "sold")}>{t("sold")}</Button><Button variant="outline" onClick={() => updateStatus(account.id, "reserved")}>{t("reserved")}</Button><Button variant="outline" onClick={() => updateStatus(account.id, "available")}>{t("available")}</Button></div><div className="mt-2 grid gap-2 min-[390px]:grid-cols-2"><Button variant="outline" onClick={() => startEdit(account)}><Pencil className="h-4 w-4" /> {t("edit")}</Button><Button variant="destructive" onClick={() => deleteStock(account.id)}><Trash2 className="h-4 w-4" /> {t("delete")}</Button></div></div>; })}</div>
                </>
              )}
            </CardContent>
          </Card>
