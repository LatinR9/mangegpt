        ...
        <Card>
          ...
          <CardContent>
            {filteredCustomers.length === 0 ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{t("empty")}</div> : (
              <>
                <div className="hidden overflow-x-auto lg:block"><Table><TableHeader><TableRow><TableHead>Customer</TableHead><TableHead>Phone</TableHead><TableHead>Line</TableHead><TableHead>Facebook</TableHead><TableHead>Telegram</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{filteredCustomers.map((customer) => <TableRow key={customer.id}><TableCell><div className="flex items-center gap-3"><img src={customer.profile_image_url ?? ""} alt="" className="h-10 w-10 rounded-full border bg-muted" /><div><p className="font-medium">{customer.nickname}</p><p className="text-sm text-muted-foreground">{customer.full_name}</p></div></div></TableCell><TableCell>{customer.phone}</TableCell><TableCell>{customer.line_id}</TableCell><TableCell className="max-w-48 truncate">{customer.facebook_url}</TableCell><TableCell>{customer.telegram_username}</TableCell><TableCell><div className="flex justify-end gap-2"><Button type="button" size="sm" variant="outline" onClick={() => startEdit(customer)}><Pencil className="h-4 w-4" /> {t("edit")}</Button><Button type="button" size="sm" variant="destructive" onClick={() => deleteCustomer(customer.id)}><Trash2 className="h-4 w-4" /> {t("delete")}</Button></div></TableCell></TableRow>)}</TableBody></Table></div>
                <div className="grid gap-3 lg:hidden">{filteredCustomers.map((customer) => <div key={customer.id} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4"><div className="flex items-center gap-3"><img src={customer.profile_image_url ?? ""} alt="" className="h-14 w-14 rounded-full border bg-muted" /><div className="min-w-0"><p className="truncate text-base font-semibold">{customer.nickname}</p><p className="truncate text-sm text-muted-foreground">{customer.full_name}</p></div></div><div className="mt-4 grid gap-2 text-sm"><p><span className="text-muted-foreground">Phone:</span> {customer.phone ?? "-"}</p><p><span className="text-muted-foreground">Line:</span> {customer.line_id ?? "-"}</p><p className="break-all"><span className="text-muted-foreground">Facebook:</span> {customer.facebook_url ?? "-"}</p><p><span className="text-muted-foreground">Telegram:</span> {customer.telegram_username ?? "-"}</p></div><div className="mt-4 grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={() => startEdit(customer)}><Pencil className="h-4 w-4" /> {t("edit")}</Button><Button type="button" variant="destructive" onClick={() => deleteCustomer(customer.id)}><Trash2 className="h-4 w-4" /> {t("delete")}</Button></div></div>)}</div>
              </>
            )}
          </CardContent>
        </Card>
