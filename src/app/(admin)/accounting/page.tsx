        ...
        <CardContent>
          {filteredTransactions.length === 0 ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{t("empty")}</div> : (
            <>
              ...
              <div className="grid gap-3 lg:hidden">{filteredTransactions.map((item) => <div key={item.id} className={cn("rounded-xl border p-4", item.type === "income" ? "border-emerald-500/30 bg-emerald-500/10" : "border-rose-500/30 bg-rose-500/10")}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><StatusBadge status={item.type === "income" ? t("income") : t("expense")} /><p className="mt-2 break-words text-base font-semibold">{item.category}</p></div><p className={item.type === "income" ? "shrink-0 text-lg font-semibold text-emerald-200" : "shrink-0 text-lg font-semibold text-rose-200"}>{formatCurrency(item.amount)}</p></div><div className="mt-3 grid gap-1 text-sm text-slate-300"><p className="break-words">{apps.find((app) => app.id === item.app_id)?.name ?? "-"} / {shareGroups.find((group) => group.id === item.group_id)?.group_name ?? "-"}</p><p>{customers.find((customer) => customer.id === item.customer_id)?.nickname ?? "-"} / {formatDate(item.date)}</p><p className="break-all">Slip: {item.slip_url ? "Attached" : "-"}</p></div><div className="mt-4 grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={() => startEdit(item)}><Pencil className="h-4 w-4" /> {t("edit")}</Button><Button type="button" variant="destructive" onClick={() => deleteTransaction(item.id)}><Trash2 className="h-4 w-4" /> {t("delete")}</Button></div></div>)}</div>
            </>
          )}
        </CardContent>
      </Card>
