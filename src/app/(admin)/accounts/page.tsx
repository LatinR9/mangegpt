        ...
        <Card className="min-w-0">
          ...
          <CardContent>
            {serviceAccounts.length === 0 ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{t("empty")}</div> : (
              <>
                <div className="hidden overflow-x-auto 2xl:block"><Table><TableHeader><TableRow><TableHead>Label</TableHead><TableHead>App</TableHead><TableHead>Type</TableHead><TableHead>Login email</TableHead><TableHead>Password</TableHead><TableHead>Expiry</TableHead><TableHead>Cost</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{serviceAccounts.map((account) => <TableRow key={account.id}><TableCell className="font-medium">{account.label}</TableCell><TableCell>{apps.find((app) => app.id === account.app_id)?.name}</TableCell><TableCell>{account.account_type === "private" ? t("private") : t("shared")}</TableCell><TableCell>{account.login_email}</TableCell><TableCell><div className="flex min-w-52 items-center gap-2"><code className="rounded-md border bg-slate-950 px-2 py-1 text-xs text-blue-100">{maskedPassword}</code><Button type="button" size="sm" variant="outline" onClick={() => copyPassword(account)}>{copiedId === account.id ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copiedId === account.id ? "Copied" : "Copy"}</Button></div></TableCell><TableCell>{formatDate(account.expiry_date)}</TableCell><TableCell>{formatCurrency(account.cost)}</TableCell><TableCell><div className="flex justify-end gap-2"><Button type="button" size="sm" variant="outline" onClick={() => startEdit(account)}><Pencil className="h-4 w-4" /> {t("edit")}</Button><Button type="button" size="sm" variant="destructive" onClick={() => deleteAccount(account.id)}><Trash2 className="h-4 w-4" /> {t("delete")}</Button></div></TableCell></TableRow>)}</TableBody></Table></div>
                <div className="grid gap-3 2xl:hidden">{serviceAccounts.map((account) => <div key={account.id} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-base font-semibold">{account.label}</p><p className="text-sm text-muted-foreground">{apps.find((app) => app.id === account.app_id)?.name} / {account.account_type === "private" ? t("private") : t("shared")}</p></div><p className="shrink-0 text-sm font-semibold text-blue-200">{formatCurrency(account.cost)}</p></div><p className="mt-3 break-all text-sm text-slate-200">{account.login_email}</p><div className="mt-3 flex items-center gap-2"><code className="min-w-0 flex-1 truncate rounded-md border bg-slate-950 px-2 py-2 text-xs text-blue-100">{maskedPassword}</code><Button type="button" variant="outline" onClick={() => copyPassword(account)}>{copiedId === account.id ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</Button></div><p className="mt-2 text-sm text-muted-foreground">Expires {formatDate(account.expiry_date)}</p><div className="mt-4 grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={() => startEdit(account)}><Pencil className="h-4 w-4" /> {t("edit")}</Button><Button type="button" variant="destructive" onClick={() => deleteAccount(account.id)}><Trash2 className="h-4 w-4" /> {t("delete")}</Button></div></div>)}</div>
              </>
            )}
          </CardContent>
        </Card>
